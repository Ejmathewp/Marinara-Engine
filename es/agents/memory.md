...
- Elige un valor de **Maximum allowed context before compression (tokens)** (contexto máximo permitido antes de comprimir, en tokens) compatible con tu modelo de chat. Es un límite antes de la compresión, no la cantidad enviada en cada turno. Incluye los tokens estimados del prompt, las herramientas, los adjuntos, el espacio de respuesta y el margen de seguridad; no es un tokenizador exacto ni un límite de facturación.
- Elige **Maximum constant summary size (tokens)** (tamaño máximo del resumen permanente, en tokens) dentro de ese límite. Cada solicitud incluye una sola instantánea breve de continuidad. Los mensajes recientes permanecen sin comprimir mientras quepa la solicitud completa.
- El **Helper model** (modelo auxiliar) toma las decisiones independientes sobre las escenas. Usa por defecto la conexión de agentes y, si no está disponible, la del chat. El procesamiento inicial del historial puede usar el modelo principal o el auxiliar; los modelos elegidos se muestran antes de preparar.
...
- **Moving context** (contexto móvil) controla los fragmentos exactos del historial. El intervalo de mensajes es **3–10** de forma predeterminada; establece ambos valores en **0** para desactivar los fragmentos, o solo el mínimo en **0** para que sean opcionales. La relevancia, el acceso del personaje y el espacio disponible pueden reducir esa cantidad, incluso a cero.
...
Para un chat existente, haz clic primero en **Prepare existing history** (preparar el historial existente). La preparación procesa el historial antiguo por lotes y muestra la fase actual junto a la rueda de hámster de Professor Mari. **Cancel** (cancelar) conserva el trabajo terminado; **Resume** (reanudar) continúa después de cerrar el panel o reiniciar el servidor. Si falla una llamada al modelo, se conserva la memoria previamente válida y se muestra un error para volver a intentarlo.

### Durante el chat

La detección de escenas se ejecuta después de la generación. Con trackers automáticos de posprocesamiento activados, las comprobaciones de escenas siguen sus llamadas y comparten la solicitud al modelo, con las mismas restricciones de visibilidad de los personajes. Sin trackers automáticos, **Standalone scene check interval (messages)** (intervalo de comprobaciones independientes de escenas, en mensajes) tiene un valor predeterminado de **5**: al alcanzar ese intervalo, el modelo auxiliar recibe solo esa ventana de mensajes recientes, sus instrucciones sobre escenas y el formato de salida. Cuentan los mensajes tanto de personas como de personajes. Las transiciones dudosas dejan la escena abierta. Si la ventana de la escena no cabe junto con la solicitud de un tracker, el tracker se ejecuta igualmente y la detección de escenas espera a una comprobación posterior.

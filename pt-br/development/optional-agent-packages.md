...
Use a declaração existente de prontidão para inicialização de forma independente quando o mundo precisar estar preparado antes do primeiro turno. Declare API 1.18 como mínimo do pacote; hosts anteriores não conseguem interpretar essa declaração de configuração. Leitores do registro que tratam a compatibilidade por entrada ignoram registros incompatíveis com um aviso em vez de bloquear outros pacotes; isso não adiciona esse tratamento retroativamente a versões anteriores do Engine.

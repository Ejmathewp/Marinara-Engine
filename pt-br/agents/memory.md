...
- Escolha um valor de **Maximum allowed context before compression (tokens)** (contexto máximo permitido antes da compressão, em tokens) compatível com o modelo do chat. Esse é o limite antes da compressão, não a quantidade enviada em cada turno. Ele inclui tokens estimados do prompt, ferramentas, anexos, espaço para a resposta e margem de segurança; não é um tokenizador exato nem um limite de cobrança.
- Escolha **Maximum constant summary size (tokens)** (tamanho máximo do resumo permanente, em tokens) dentro desse limite. Cada solicitação inclui uma única visão breve da continuidade. Mensagens recentes permanecem sem compressão enquanto a solicitação inteira couber.
- O **Helper model** (modelo auxiliar) toma as decisões independentes sobre cenas. Por padrão, usa a conexão dos agentes e, se ela não estiver disponível, a do chat. O processamento inicial do histórico pode usar o modelo principal ou auxiliar; os modelos definidos aparecem antes da preparação.
...
- **Moving context** (contexto móvel) controla trechos exatos do histórico. O intervalo de mensagens é **3–10** por padrão; defina os dois valores como **0** para desativar os trechos, ou apenas o mínimo como **0** para torná-los opcionais. Relevância, acesso do personagem e espaço disponível podem resultar em menos mensagens, inclusive nenhuma.
...
Em um chat existente, clique primeiro em **Prepare existing history** (preparar o histórico existente). A preparação processa o histórico antigo em lotes e mostra a etapa atual ao lado da roda de hamster de Professor Mari. **Cancel** (cancelar) preserva o trabalho concluído; **Resume** (retomar) continua depois de fechar o painel ou reiniciar o servidor. Uma chamada de modelo que falha preserva a memória anteriormente válida e mostra um erro para tentar novamente.

### Durante o chat

A detecção de cenas ocorre depois da geração. Com trackers automáticos de pós-processamento ativados, as verificações de cena seguem as chamadas dos trackers e compartilham sua solicitação ao modelo, com as mesmas restrições de visibilidade dos personagens. Sem trackers automáticos, **Standalone scene check interval (messages)** (intervalo de verificações independentes de cena, em mensagens) é **5** por padrão: ao atingir esse intervalo, o modelo auxiliar recebe apenas essa janela de mensagens recentes, suas instruções de cena e o formato de saída. Mensagens de personas e de personagens contam. Transições incertas mantêm a cena aberta. Se a janela da cena não couber junto com a solicitação de um tracker, o tracker ainda é executado e a detecção de cenas aguarda uma verificação posterior.

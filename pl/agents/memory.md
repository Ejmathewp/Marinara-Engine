...
- Wybierz **Maximum allowed context before compression (tokens)** (maksymalny kontekst przed kompresją w tokenach) obsługiwany przez model czatu. To górna granica przed kompresją, a nie ilość danych wysyłana w każdej turze. Limit obejmuje szacowane tokeny promptu, narzędzia, załączniki, miejsce na odpowiedź i margines bezpieczeństwa; nie jest dokładnym tokenizerem ani limitem rozliczeń.
- W ramach tego limitu wybierz **Maximum constant summary size (tokens)** (maksymalny rozmiar stałego podsumowania w tokenach). Każde żądanie zawiera jedną krótką migawkę ciągłości. Niedawne wiadomości pozostają nieskompresowane, dopóki całe żądanie mieści się w limicie.
- **Helper model** (model pomocniczy) samodzielnie podejmuje decyzje o scenach. Domyślnie używa połączenia agentów, a gdy go brak – połączenia czatu. Początkowe przetwarzanie historii może korzystać z modelu głównego lub pomocniczego; wybrane modele są pokazane przed przygotowaniem.
...
- **Moving context** (ruchomy kontekst) kontroluje dokładne fragmenty wcześniejszych wiadomości. Domyślny zakres to **3–10** wiadomości; ustaw obie wartości na **0**, aby wyłączyć fragmenty, albo tylko minimum na **0**, aby stały się opcjonalne. Trafność, dostęp postaci i wolne miejsce mogą ograniczyć liczbę wiadomości, także do zera.
...
W istniejącym czacie najpierw kliknij **Prepare existing history** (przygotuj dotychczasową historię). Przygotowanie przetwarza starszą historię partiami i pokazuje bieżący etap obok kołowrotka chomika Professor Mari. **Cancel** (anuluj) zachowuje ukończoną pracę; **Resume** (wznów) kontynuuje ją po zamknięciu panelu lub ponownym uruchomieniu serwera. Nieudane wywołanie modelu zachowuje wcześniejszą poprawną pamięć i pokazuje błąd z możliwością ponowienia.

### Podczas rozmowy

Wykrywanie scen odbywa się po generowaniu. Przy włączonych automatycznych trackerach przetwarzania końcowego sprawdzanie scen następuje przy ich wywołaniach i korzysta z tego samego żądania modelu oraz tych samych ograniczeń widoczności dla postaci. Bez automatycznych trackerów **Standalone scene check interval (messages)** (odstęp między samodzielnymi sprawdzeniami scen w wiadomościach) wynosi domyślnie **5**: po osiągnięciu tego odstępu model pomocniczy otrzymuje tylko to okno ostatnich wiadomości, instrukcje dotyczące scen i format odpowiedzi. Liczą się wiadomości person i postaci. Niepewne przejścia pozostawiają scenę otwartą. Jeśli okno sceny nie mieści się w żądaniu trackera, tracker nadal działa, a wykrywanie scen czeka na kolejne sprawdzenie.

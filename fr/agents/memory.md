...
- Choisis une valeur de **Maximum allowed context before compression (tokens)** (contexte maximal avant compression, en tokens) prise en charge par ton modèle de chat. C'est un plafond avant compression, pas la quantité envoyée à chaque tour. Il comprend les tokens estimés du prompt, les outils, les pièces jointes, la place pour la réponse et une marge de sécurité ; ce n'est ni un comptage exact des tokens ni une limite de facturation.
- Choisis **Maximum constant summary size (tokens)** (taille maximale du résumé permanent, en tokens) à l'intérieur de ce plafond. Un bref état de la continuité est inclus une seule fois dans chaque requête. Les messages récents restent intacts tant que la requête entière tient dans la limite.
- Le **Helper model** (modèle auxiliaire) prend les décisions autonomes sur les scènes. Il utilise par défaut la connexion des agents, puis celle du chat en l'absence de la première. Le traitement initial de l'historique peut utiliser le modèle principal ou auxiliaire ; les modèles retenus sont affichés avant la préparation.
...
- **Moving context** (contexte mobile) contrôle les extraits exacts de l'historique. La plage de messages est de **3–10** par défaut ; règle les deux valeurs sur **0** pour désactiver les extraits, ou seulement le minimum sur **0** pour les rendre facultatifs. La pertinence, les droits du personnage et l'espace disponible peuvent réduire leur nombre, jusqu'à zéro.
...
Pour un chat existant, clique d'abord sur **Prepare existing history** (préparer l'historique existant). La préparation parcourt l'ancien historique par lots et affiche l'étape actuelle à côté de la roue de hamster de Professor Mari. **Cancel** (annuler) conserve le travail terminé ; **Resume** (reprendre) le poursuit après la fermeture du panneau ou le redémarrage du serveur. Un appel de modèle qui échoue conserve la mémoire auparavant valide et affiche une erreur permettant de réessayer.

### Pendant le chat

La détection des scènes a lieu après la génération. Lorsque des trackers automatiques de post-traitement sont activés, les vérifications de scènes suivent leurs appels et partagent leur requête au modèle, avec les mêmes restrictions de visibilité des personnages. Sans tracker automatique, **Standalone scene check interval (messages)** (intervalle des vérifications autonomes de scènes, en messages) vaut **5** par défaut : une fois cet intervalle atteint, le modèle auxiliaire reçoit uniquement cette fenêtre de messages récents, ses instructions sur les scènes et le format de sortie. Les messages des personas et des personnages comptent tous. Les transitions incertaines laissent la scène ouverte. Si la fenêtre de scène ne tient pas dans la requête d'un tracker, le tracker s'exécute quand même et la détection des scènes attend une vérification ultérieure.

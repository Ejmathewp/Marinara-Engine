...
- Wähle für **Maximum allowed context before compression (tokens)** (maximaler Kontext vor der Komprimierung in Tokens) einen Wert, den dein Chatmodell unterstützt. Dies ist die Obergrenze vor der Komprimierung, nicht die in jeder Runde gesendete Textmenge. Sie umfasst geschätzte Prompt-Tokens, Tools, Anhänge, Antwortplatz und Sicherheitsreserve; sie ist kein exakter Tokenizer oder Abrechnungslimit.
- Wähle innerhalb dieser Grenze **Maximum constant summary size (tokens)** (maximale Größe der dauerhaften Zusammenfassung in Tokens). Eine kurze Kontinuitätsübersicht wird einmal pro Anfrage eingefügt. Jüngste Nachrichten bleiben ungekürzt, solange die gesamte Anfrage hineinpasst.
- Das **Helper model** (Hilfsmodell) trifft eigenständige Szenenentscheidungen. Standardmäßig nutzt es die Agentenverbindung, ersatzweise die Chatverbindung. Die erste Verarbeitung des Verlaufs kann das Haupt- oder Hilfsmodell verwenden; die tatsächlich gewählten Modelle werden vor der Vorbereitung angezeigt.
...
- **Moving context** (beweglicher Kontext) steuert wortgetreue Verlaufsauszüge. Der Nachrichtenbereich beträgt standardmäßig **3–10**; setze beide Werte auf **0**, um Auszüge zu deaktivieren, oder nur das Minimum auf **0**, um sie optional zu machen. Relevanz, Charakterzugriff und verfügbarer Platz können zu weniger Nachrichten führen, auch zu keiner.
...
Klicke bei einem bestehenden Chat zuerst auf **Prepare existing history** (bisherigen Verlauf vorbereiten). Die Vorbereitung verarbeitet ältere Nachrichten stapelweise und zeigt den aktuellen Schritt neben Professor Maris Hamsterrad. **Cancel** (Abbrechen) behält abgeschlossene Arbeit; **Resume** (Fortsetzen) macht nach dem Schließen der Seitenleiste oder einem Serverneustart weiter. Ein fehlgeschlagener Modellaufruf bewahrt bisher gültige Erinnerungen und zeigt einen Fehler zum erneuten Versuch.

### Während des Chats

Die Szenenerkennung läuft nach der Generierung. Sind automatische Tracker für die Nachbearbeitung aktiviert, folgen Szenenprüfungen deren Aufrufen und nutzen dieselbe Modellanfrage mit denselben Sichtbarkeitsbeschränkungen für Charaktere. Ohne automatische Tracker beträgt **Standalone scene check interval (messages)** (Intervall eigenständiger Szenenprüfungen in Nachrichten) standardmäßig **5**: Sobald das Intervall erreicht ist, erhält das Hilfsmodell nur dieses jüngste Nachrichtenfenster, seine Szenenanweisungen und das Ausgabeformat. Persona- und Charakternachrichten zählen gleichermaßen. Bei unklaren Übergängen bleibt die Szene offen. Passt das Szenenfenster nicht zusätzlich in eine Tracker-Anfrage, läuft der Tracker trotzdem; die Szenenerkennung wartet auf eine spätere Prüfung.

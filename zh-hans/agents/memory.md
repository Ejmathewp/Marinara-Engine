...
- 在聊天模型支持的范围内选择 **Maximum allowed context before compression (tokens)**(压缩前允许的最大上下文，token)。这是开始压缩前的上限，不是每轮发送的数量。上限包含预估的提示词 token、工具、附件、回复空间和安全余量，不是精确的分词计数或计费上限。
- 在这个上限内选择 **Maximum constant summary size (tokens)**(常驻摘要最大长度，token)。每次请求只会加入一次简短的剧情连续性快照。只要完整请求仍能容纳，近期消息就保持原文，不做压缩。
- **Helper model**(辅助模型)负责独立运行的场景判断。默认使用智能体连接，无法使用时回退到聊天连接。首次处理历史时可以使用主模型或辅助模型；实际选定的模型会在准备前显示。
- 场景摘要和压缩使用现有的 **Summaries**(摘要)模型和提示词。Advanced Memory 独立于智能体总开关运行，不需要下载智能体。
- **Moving context**(可变上下文)控制历史消息的原文片段。消息范围默认为 **3–10** 条；将两个值都设为 **0** 可以禁用片段，只将最小值设为 **0** 则允许按需省略片段。根据相关性、角色访问范围和剩余空间，实际可能更少，也可能一条都没有。
...
对于已有聊天，先点击 **Prepare existing history**(准备已有历史)。准备过程会分批处理旧历史，并在 Professor Mari 的仓鼠跑轮旁显示当前阶段。**Cancel**(取消)会保留已经完成的工作；关闭抽屉或重启服务器后，仍可通过 **Resume**(继续)接着处理。模型调用失败时，会保留此前有效的记忆，并显示可重试的错误。

### 聊天过程

场景检测在生成后运行。启用自动后处理追踪器时，场景检查跟随追踪器调用，共用现有的模型请求，并遵守相同的角色可见性限制。没有自动追踪器时，**Standalone scene check interval (messages)**(独立场景检查间隔，消息数)默认为 **5**：达到间隔后，辅助模型只会收到对应的近期消息窗口、场景判断指令和输出格式。用户角色和角色的消息都会计入。无法确定是否发生转场时，场景保持开放。如果场景窗口无法与追踪器请求一起放入上下文，追踪器仍会运行，场景检测则等待后续检查。

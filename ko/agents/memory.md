...
- 채팅 모델이 지원하는 범위 안에서 **Maximum allowed context before compression (tokens)**(압축 전 최대 허용 컨텍스트, 토큰)를 선택하세요. 매 턴 보내는 양이 아니라 압축 전의 상한입니다. 한도에는 예상 프롬프트 토큰, 도구, 첨부 파일, 답변 공간, 안전 여유분이 포함됩니다. 정확한 토크나이저 계산이나 과금 한도는 아닙니다.
- 이 한도 안에서 **Maximum constant summary size (tokens)**(상시 요약 최대 크기, 토큰)를 선택하세요. 연속성을 위한 짧은 스냅샷을 요청마다 한 번 포함합니다. 전체 요청이 한도 안에 들어가는 동안 최근 메시지는 압축하지 않습니다.
- **Helper model**(보조 모델)은 트래커와 별도로 실행하는 장면 판정을 담당합니다. 기본값은 에이전트 연결이며, 사용할 수 없으면 채팅 연결을 사용합니다. 초기 기록 처리에는 기본 모델이나 보조 모델을 사용할 수 있으며, 실제 사용할 모델은 준비 전에 표시됩니다.
- 장면 요약과 압축은 기존 **Summaries**(요약) 모델과 프롬프트를 사용합니다. Advanced Memory는 메인 Agents 스위치와 독립적으로 작동하며 다운로드하는 에이전트가 필요하지 않습니다.
- **Moving context**(가변 컨텍스트)는 과거 메시지의 원문 발췌를 제어합니다. 메시지 범위의 기본값은 **3–10**개입니다. 두 값을 모두 **0**으로 설정하면 발췌를 끄고, 최솟값만 **0**으로 설정하면 필요할 때만 포함합니다. 관련성, 캐릭터 접근 범위, 남은 공간에 따라 더 적게 포함되거나 하나도 포함되지 않을 수 있습니다.
...
기존 채팅에서는 먼저 **Prepare existing history**(기존 기록 준비)를 클릭하세요. 준비 과정은 과거 기록을 묶음으로 처리하고 Professor Mari의 햄스터 쳇바퀴 옆에 현재 단계를 표시합니다. **Cancel**(취소)을 눌러도 완료한 작업은 유지됩니다. 서랍을 닫거나 서버를 재시작한 뒤에도 **Resume**(재개)으로 이어서 처리할 수 있습니다. 모델 호출이 실패하면 이전에 유효했던 기억을 유지하고 재시도할 수 있는 오류를 표시합니다.

### 채팅 중 동작

장면 감지는 생성 후에 실행됩니다. 자동 후처리 트래커가 켜져 있으면 장면 확인도 트래커 호출에 맞춰 실행하고 기존 모델 요청을 함께 사용합니다. 캐릭터별 표시 제한도 동일하게 적용됩니다. 자동 트래커가 없으면 **Standalone scene check interval (messages)**(독립 장면 확인 간격, 메시지)의 기본값은 **5**입니다. 간격에 도달하면 보조 모델에는 해당 최근 메시지 범위, 장면 지침, 출력 형식만 보냅니다. 페르소나와 캐릭터의 메시지를 모두 셉니다. 장면 전환이 불확실하면 현재 장면을 열린 상태로 둡니다. 장면 확인에 필요한 메시지를 트래커 요청과 함께 넣을 공간이 부족해도 트래커는 실행되며, 장면 감지는 다음 확인 기회까지 기다립니다.

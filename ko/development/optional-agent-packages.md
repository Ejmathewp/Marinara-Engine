...
첫 턴 전에 월드를 준비해야 한다면 기존 시작 준비 선언을 별도로 사용하세요. 패키지의 최소 버전으로 API 1.18을 선언하세요. 이전 호스트는 이 설정 선언을 해석하지 못합니다. 항목별 호환성 처리를 갖춘 레지스트리 리더는 다른 패키지를 차단하지 않고 지원하지 않는 레코드를 경고와 함께 건너뜁니다. 이 처리가 과거 Engine 릴리스에 소급해서 추가되는 것은 아닙니다.
